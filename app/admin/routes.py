...
def show_admins(username):
    ...
    admins_names = mongo.db.users.find({'$and': [{'username': {'$ne': username}}, {'role': 'admin'}]},
                                       {'name': 1, 'username': 1})
    return render_template('admin/show_admins.html', user=user, admins_details=admins_details,
...
def create_admin(username):
    ...
    return render_template('admin/create_admin.html', user=user, admins_details=admins_details,
                           admins_names=admins_names)
...
def edit_other_admin(username, other_admin_username):
    ...
    if request.method == 'POST':
        ...
        if new_password == re_new_password:
            mongo.db.users.update_one({'username': other_admin['username']}, {'$set': {'password': new_password}},
                                      upsert=False)
